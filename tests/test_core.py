"""Tests for JEPA-LM core components."""
...
class TestEncoder:
    def test_forward(self, tiny_config):
        enc = BidirectionalEncoder(
            vocab_size=tiny_config.enc_vocab_size,
            hidden_dim=tiny_config.enc_hidden_dim,
            num_layers=tiny_config.enc_num_layers,
            num_heads=tiny_config.enc_num_heads,
            ff_dim=tiny_config.enc_ff_dim,
            max_seq_len=tiny_config.enc_max_seq_len,
        )
        x = torch.randint(0, 1000, (2, 32))
        out = enc(x)
        assert out.shape == (2, 32, tiny_config.enc_hidden_dim)

    def test_output_is_not_collapsed(self, tiny_config):
        enc = BidirectionalEncoder(
            vocab_size=tiny_config.enc_vocab_size,
            hidden_dim=tiny_config.enc_hidden_dim,
            num_layers=tiny_config.enc_num_layers,
...
class TestPredictor:
    def test_forward(self, tiny_config):
        pred = Predictor(
            enc_hidden_dim=tiny_config.enc_hidden_dim,
            pred_hidden_dim=tiny_config.pred_hidden_dim,
            pred_num_layers=tiny_config.pred_num_layers,
            pred_num_heads=tiny_config.pred_num_heads,
...
class TestDecoder:
    def test_forward(self, tiny_config):
        dec = LightweightDecoder(
            vocab_size=tiny_config.enc_vocab_size,
            hidden_dim=tiny_config.dec_hidden_dim,
            num_layers=tiny_config.dec_num_layers,
            num_heads=tiny_config.dec_num_heads,
            ff_dim=tiny_config.dec_ff_dim,
            max_seq_len=tiny_config.enc_max_seq_len,
        )
        x = torch.randint(0, 1000, (2, 16))
        logits = dec(x)
...
class TestMasking:
    def test_create_span_mask(self, tiny_config):
        result = create_span_mask(
            ...
            mask_prob=tiny_config.mask_prob,
            mean_span_length=tiny_config.mean_span_length,
...
class TestLoss:
    ...
    def test_total_loss(self):
        j = torch.tensor(1.0)
        n = torch.tensor(0.5)
        t = total_loss(j, n, lambda_jepa=1.0, lambda_ntp=0.1)
